...
Json measure(std::string_view id, std::size_t input_bytes, Operation operation) {
  for (std::size_t index = 0; index < 10U; ++index) { operation(); }
  std::vector<double> samples;
  samples.reserve(kSamples);
  for (std::size_t sample = 0; sample < kSamples; ++sample) {
    const auto started = Clock::now();
    for (std::size_t iteration = 0; iteration < kIterations; ++iteration) { operation(); }
...
int main(int argc, char** argv) {
  try {
    ...
    cases.push_back(measure("component.json_sax.valid", valid.size(), [&] { require(native_mcp::preflight_json(valid) == native_mcp::JsonPreflightStatus::kOk, "preflight"); }));
    cases.push_back(measure("component.json_sax.rejected_duplicate", rejected.size(), [&] { require(native_mcp::preflight_json(rejected) == native_mcp::JsonPreflightStatus::kDuplicateKey, "reject"); }));
    cases.push_back(measure("component.json_dom.closed_schema", valid.size(), [&] { const Json value = Json::parse(valid); require(value.size() == 4U && value.at("method") == "tools/list", "dom"); }));
    cases.push_back(measure("component.runtime_policy.parse", config.size(), [&] { require(native_mcp::parse_runtime_policy_config(config).config.has_value(), "policy"); }));
    cases.push_back(measure("component.proc_text.parse", stat.size() + status.size(), [&] { require(native_mcp::process_parsing::parse_stat_identity_text(stat).has_value() && native_mcp::process_parsing::parse_status_text(status).has_value(), "proc"); }));
    ...
    if (policy.policy.has_value()) {
      ...
      cases.push_back(measure("component.logs.tail.streaming", 56U, [&] { auto opened = policy.policy->open_regular_file("fixtures", "log.txt"); require(opened.file.has_value(), "open log"); auto result = logs.tail(*opened.file, {.max_lines=2U}); require(result.result.has_value() && result.result->lines.size() == 2U, "tail"); }));
      cases.push_back(measure("component.elf.inspect", 64U, [&] { auto opened = policy.policy->open_regular_file("fixtures", "minimal.elf"); require(opened.file.has_value(), "open elf"); auto result = elf.inspect(*opened.file); require(result.result.has_value(), "elf"); }));
