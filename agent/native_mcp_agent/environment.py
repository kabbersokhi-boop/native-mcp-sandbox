...
def _validate_name(name: str) -> None:
    if not isinstance(name, str) or not _NAME.fullmatch(name):
        raise ProviderError(
            failure(
                FailureClass.LOCAL_VALIDATION_FAILURE, "invalid child environment variable name"
            )
        )
...
def _validate_proxy(value: str) -> None:
    if (
        ...
        raise ProviderError(
            failure(FailureClass.LOCAL_VALIDATION_FAILURE, "proxy value is invalid")
        )
...
def _validate_no_proxy(value: str) -> None:
    if (
        ...
        raise ProviderError(
            failure(FailureClass.LOCAL_VALIDATION_FAILURE, "NO_PROXY value is invalid")
        )
    entries = value.split(",")
    ...
    for entry in entries:
        ...
        if entry.startswith("["):
            ...
            try:
                ...
                valid_host = _valid_dns_name(host_value) and (
                    host.startswith(".") or not host.startswith(".")
                )
            if not valid_host:
                raise ProviderError(
                    failure(FailureClass.LOCAL_VALIDATION_FAILURE, "NO_PROXY value is invalid")
...
def build_child_environment(
    ...
    *,
    ...
    if (
        ...
        or max_value_bytes > 64 * 1024
    ):
        raise ProviderError(
            failure(FailureClass.LOCAL_VALIDATION_FAILURE, "environment value limit is invalid")
        )
    if not isinstance(provider_child, bool) or (allow_proxy and provider_child):
        raise ProviderError(
            failure(
                FailureClass.LOCAL_POLICY_FAILURE,
                "proxy allowlisting is only for non-provider children",
            )
        )
    if not isinstance(parent, Mapping):
        raise ProviderError(
            failure(FailureClass.LOCAL_VALIDATION_FAILURE, "parent environment is invalid")
        )
    for inherited_name, inherited_value in parent.items():
        ...
        if not isinstance(inherited_value, str) or "\x00" in inherited_value:
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_VALIDATION_FAILURE, "inherited environment value is invalid"
                )
            )
    if not isinstance(allowlist, (tuple, list)) or isinstance(allowlist, str):
        raise ProviderError(
            failure(FailureClass.LOCAL_VALIDATION_FAILURE, "child environment allowlist is invalid")
        )
    ...
    if len(set(names)) != len(names):
        raise ProviderError(
            failure(FailureClass.LOCAL_VALIDATION_FAILURE, "allowlist contains duplicate names")
        )
    if required is not None and not isinstance(required, Mapping):
        raise ProviderError(
            failure(
                FailureClass.LOCAL_VALIDATION_FAILURE, "required environment values are invalid"
            )
        )
    try:
    ...
    if set(values) - set(names):
        raise ProviderError(
            failure(FailureClass.LOCAL_VALIDATION_FAILURE, "required variable is not allowlisted")
        )
    result: dict[str, str] = {}
        ...
        if upper in _NEVER_ALLOW or any(hint in upper for hint in _SECRET_HINTS + _PROVIDER_HINTS):
            if upper in {"HTTP_PROXY", "HTTPS_PROXY", "ALL_PROXY", "NO_PROXY"} and allow_proxy:
                ...
                raise ProviderError(
                    failure(
                        FailureClass.LOCAL_POLICY_FAILURE,
                        "secret or provider variable cannot be allowlisted",
                    )
                )
        ...
        if not isinstance(value, str) or "\x00" in value:
            raise ProviderError(
                failure(
                    ...
                    "child environment value is invalid or oversized",
                )
            )
