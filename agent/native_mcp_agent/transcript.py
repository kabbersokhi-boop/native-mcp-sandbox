...
_TRANSCRIPT_SCHEMA = {
    "process_start": set(),
    "initialize_request": set(),
    "initialize_response": {"response"},
    "initialized_notification": set(),
    "tools_list_request": set(),
    "tools_list_response": {"response"},
    "surface_captured": {"surface"},
    "surface_revalidated": {"surface"},
    "provider_turn_start": {"turn", "bytes"},
    "provider_turn_response": {"turn", "bytes"},
    "proposal_rejected": {"proposal"},
    "proposal_duplicate": {"proposal"},
    "authorized": {"action", "proposal"},
    "mcp_request": {"action", "response"},
    "mcp_response": {"action", "response"},
    "evidence_validated": {"action", "response"},
    "skipped": {"proposal"},
    "deadline": set(),
    "cancelled": set(),
    "failure": {"failure"},
    "shutdown_start": set(),
    "shutdown_terminate": set(),
    "shutdown_kill": set(),
    "shutdown_complete": set(),
    "shutdown_unreaped": set(),
    "outcome": {"outcome"},
    "transcript_limit": set(),
    # Compatibility aliases emitted by the initial transcript correction.
    "surface": {"surface"},
    "provider_turn": {"turn", "bytes"},
...
def _safe_identity(value: Any, label: str) -> str:
    ...
    if not isinstance(value, str) or not _SAFE_ID.fullmatch(value) or size < 1 or size > 64:
        raise ProviderError(
            failure(FailureClass.LOCAL_VALIDATION_FAILURE, f"transcript {label} is invalid")
        )
    lowered = value.lower()
...
def _metadata(value: Any, *, limit: int) -> Mapping[str, str]:
    if not isinstance(value, Mapping) or len(value) > limit:
        raise ProviderError(
            failure(FailureClass.LOCAL_VALIDATION_FAILURE, "transcript metadata is invalid")
        )
    result: dict[str, str] = {}
    for key, item in value.items():
        ...
        if (
            not item
            or size > HARD_LIMITS.message_bytes
        ...
        lowered = item.lower()
        if (
            any(marker in lowered for marker in _SUSPICIOUS)
            or item.startswith("/")
...
class TranscriptEvent:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise ProviderError(
                failure(FailureClass.LOCAL_VALIDATION_FAILURE, "transcript model is invalid")
            ) from error
        object.__setattr__(self, "model", model)
        if not isinstance(self.correlation_id, RequestCorrelationId) or not isinstance(
            self.provenance, EvidenceProvenance
        ):
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_VALIDATION_FAILURE,
                    "transcript identity is not project-owned",
                )
            )
        if (
            not isinstance(self.proposal_ids, tuple)
            or len(self.proposal_ids) > HARD_LIMITS.proposed_tool_call_count
            or any(not isinstance(item, ToolCallId) for item in self.proposal_ids)
            or len(set(self.proposal_ids)) != len(self.proposal_ids)
        ):
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_VALIDATION_FAILURE, "transcript proposal IDs are invalid"
                )
            )
        if self.failure_class is not None and not isinstance(self.failure_class, FailureClass):
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_VALIDATION_FAILURE, "transcript failure class is invalid"
                )
            )
        if self.retry_eligible is not None and not isinstance(self.retry_eligible, bool):
            raise ProviderError(
                failure(FailureClass.LOCAL_VALIDATION_FAILURE, "transcript retry flag is invalid")
            )
        if (
            not isinstance(self.byte_count, int)
            or isinstance(self.byte_count, bool)
            or not 0 <= self.byte_count <= 512 * 1024
        ):
            raise ProviderError(
                failure(FailureClass.LOCAL_VALIDATION_FAILURE, "transcript byte count is invalid")
            )
        object.__setattr__(
            self, "metadata", _metadata(self.metadata, limit=HARD_LIMITS.object_array_items)
        )
    ...
    def to_json_bytes(self, limits: Limits = DEFAULT_LIMITS) -> bytes:
        ...
        if not isinstance(self.correlation_id, RequestCorrelationId):
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_VALIDATION_FAILURE, "transcript correlation ID is invalid"
                )
            )
        RequestCorrelationId(str(self.correlation_id))
        if not isinstance(self.provenance, EvidenceProvenance):
            raise ProviderError(
                failure(FailureClass.LOCAL_VALIDATION_FAILURE, "transcript provenance is invalid")
            )
        ...
        if len(self.proposal_ids) > limits.proposed_tool_call_count:
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_VALIDATION_FAILURE,
                    "transcript proposal count exceeds configured limit",
                )
            )
        ...
        if any(len(value.encode("utf-8")) > limits.message_bytes for value in metadata.values()):
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_VALIDATION_FAILURE,
                    "transcript metadata exceeds configured limit",
                )
            )
        value: dict[str, Any] = {
            "schemaVersion": 1,
        ...
        if len(encoded) > limits.transcript_bytes:
            raise ProviderError(
                failure(FailureClass.OVERSIZED_RESPONSE, "transcript exceeds byte limit")
            )
        return encoded
...
def parse_transcript(raw: bytes | str, limits: Limits = DEFAULT_LIMITS) -> TranscriptEvent:
    ...
    if not isinstance(raw, (bytes, str)):
        raise ProviderError(
            failure(FailureClass.MALFORMED_JSON, "transcript input type is invalid")
        )
    try:
        encoded = raw if isinstance(raw, bytes) else raw.encode("utf-8")
    except UnicodeEncodeError:
        raise ProviderError(
            failure(FailureClass.MALFORMED_JSON, "transcript input is not valid UTF-8")
        ) from None
    if len(encoded) > limits.transcript_bytes:
        raise ProviderError(
            failure(FailureClass.OVERSIZED_RESPONSE, "transcript exceeds byte limit")
    ...
    try:
        ...
        if not isinstance(value, dict):
            raise ProviderError(
                failure(FailureClass.LOCAL_VALIDATION_FAILURE, "transcript must be an object")
            )
        allowed = {
            "schemaVersion",
            "event",
        ...
        if (
            ...
            raise ProviderError(
                failure(FailureClass.LOCAL_VALIDATION_FAILURE, "transcript schema is not closed")
            )
        if (
            not isinstance(value["proposalIds"], list)
            or len(value["proposalIds"]) > limits.proposed_tool_call_count
        ):
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_VALIDATION_FAILURE, "transcript proposal count is invalid"
                )
            )
        if (
            not isinstance(value["byteCount"], int)
            or isinstance(value["byteCount"], bool)
        ...
        return TranscriptEvent(
            ...
            value["adapter"],
            value["model"],
            RequestCorrelationId(value["correlationId"]),
            EvidenceProvenance(value["provenance"]),
            proposals,
            failure_class,
            retry_eligible,
            value["byteCount"],
        ...
        raise ProviderError(
            failure(FailureClass.LOCAL_VALIDATION_FAILURE, "transcript fields are malformed")
        ) from None


class BoundedTranscript:
    """Incrementally bounded, closed control transcript."""
    ...
    def add(self, event: str, **metadata: str) -> None:
        if self._limited:
            return
        if event not in _TRANSCRIPT_EVENTS or (
            set(metadata) != _TRANSCRIPT_SCHEMA[event]
            and not (event == "failed" and set(metadata) == {"failure"})
        ):
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_VALIDATION_FAILURE, "control transcript event is invalid"
                )
            )
        values: dict[str, str] = {}
        for key, value in metadata.items():
            if (
                ...
                raise ProviderError(
                    failure(
                        FailureClass.LOCAL_VALIDATION_FAILURE,
                        "control transcript metadata is invalid",
                    )
                )
            if (
                any(part in value.lower() for part in _SUSPICIOUS)
                or value.startswith("/")
                or "\\" in value
            ):
                values[key] = "redacted"
        ...
        if (
            event == "transcript_limit"
            or len(_transcript_bytes(candidate + [self._terminal], True))
            <= self._limits.transcript_bytes
        ):
            self._events = candidate
            return
        terminal = self._events + [self._terminal]
        if len(_transcript_bytes(terminal, True)) > self._limits.transcript_bytes:
            raise ProviderError(
                failure(FailureClass.OVERSIZED_RESPONSE, "transcript terminal cannot fit")
            )
        self._events = terminal
        self._limited = True

    def to_json_bytes(self) -> bytes:
        return _transcript_bytes(self._events, self._limited)


def _transcript_bytes(events: list[dict[str, Any]], limited: bool) -> bytes:
    return json.dumps(
        {"schemaVersion": 2, "events": events, "limited": limited},
        ensure_ascii=True,
        sort_keys=True,
        separators=(",", ":"),
    ).encode("utf-8")
...
def parse_bounded_transcript(
    raw: bytes | str, limits: Limits = DEFAULT_LIMITS
) -> tuple[Mapping[str, Any], ...]:
    value = parse_closed_json(raw, limits, byte_limit=limits.transcript_bytes)
    if (
        not isinstance(value, dict)
        or set(value) != {"schemaVersion", "events", "limited"}
        or value["schemaVersion"] != 2
        or type(value["limited"]) is not bool
        or not isinstance(value["events"], list)
    ):
        raise ProviderError(
            failure(FailureClass.LOCAL_VALIDATION_FAILURE, "control transcript is not closed")
        )
    result = []
    ...
    for item in value["events"]:
        if (
            ...
            or set(item) != {"event", "metadata"}
            or item["event"] not in _TRANSCRIPT_EVENTS
            or not isinstance(item["metadata"], dict)
            or (
                set(item["metadata"]) != _TRANSCRIPT_SCHEMA[item["event"]]
                and not (item["event"] == "failed" and set(item["metadata"]) == {"failure"})
            )
        ):
            raise ProviderError(
                failure(
        ...
        if any(not isinstance(v, str) or len(v) > 64 for v in item["metadata"].values()):
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_VALIDATION_FAILURE, "control transcript metadata is invalid"
                )
            )
        ...
        if event == "authorized":
            ...
            if action in authorized_actions:
                raise ProviderError(
                    failure(
                        FailureClass.LOCAL_VALIDATION_FAILURE,
                        "control transcript authorization is stale",
                    )
                )
            ...
            if (
                pair[0] not in authorized_actions
                or pair in requested_pairs
                or pair[0] in request_actions
                or pair[1] in request_responses
            ):
                raise ProviderError(
                    failure(
                        FailureClass.LOCAL_VALIDATION_FAILURE,
                        "control transcript request provenance is invalid",
                    )
                )
            requested_pairs.add(pair)
            request_actions.add(pair[0])
            request_responses.add(pair[1])
            ...
            if pair not in requested_pairs or pair in responded_pairs:
                raise ProviderError(
                    failure(
                        FailureClass.LOCAL_VALIDATION_FAILURE,
                        "control transcript response provenance is invalid",
                    )
                )
            ...
            if pair not in responded_pairs or pair in evidenced_pairs:
                raise ProviderError(
                    failure(
                        FailureClass.LOCAL_VALIDATION_FAILURE,
                        "control transcript evidence provenance is invalid",
                    )
                )
            evidenced_pairs.add(pair)
        result.append(_FrozenDict(item))
    terminal_indexes = [
        index for index, item in enumerate(result) if item["event"] == "transcript_limit"
    ]
    ...
    if value["limited"]:
        if terminal_indexes != [len(result) - 1]:
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_VALIDATION_FAILURE, "control transcript terminal is invalid"
                )
            )
    elif terminal_indexes:
        raise ProviderError(
            failure(FailureClass.LOCAL_VALIDATION_FAILURE, "control transcript terminal is invalid")
        )
    return tuple(result)
