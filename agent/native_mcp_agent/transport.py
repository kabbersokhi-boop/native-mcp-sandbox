...
class LoopbackFakeTransport:
    ...
    def send(
        ...
        *,
        ...
        if not isinstance(correlation_id, str) or str(request.correlation_id) != correlation_id:
            raise ProviderError(
                failure(FailureClass.LOCAL_VALIDATION_FAILURE, "transport correlation ID mismatch")
            )
        try:
            ...
            raise ProviderError(
                failure(FailureClass.LOCAL_VALIDATION_FAILURE, "provider request is invalid")
            ) from None
            ...
            if (
                ...
                raise ProviderError(
                    failure(
                        ...
                        "provider deadline is invalid or expired",
                    )
                )
        ...
        for attempt in range(1, limits.provider_attempt_count + 1):
            ...
            last = None
            try:
                response_body, content_type, status, retry_after = self._one_attempt(
                    endpoint, body, limits, deadline_at
                )
                if 300 <= status <= 399:
                    ...
                    if (
                        ...
                        raise ProviderError(
                            failure(
                                ...
                                "provider did not return application/json",
                                attempt=attempt,
                            )
                        )
                    return parse_provider_response(
                        response_body, advertised_tools=request.tools, limits=limits
                    )
                ...
                last = failure(
                    classification, "provider read inactivity deadline expired", attempt=attempt
                )
                ...
                last = failure(
                    FailureClass.MALFORMED_HTTP_RESPONSE,
            ...
            if decision.delay_ms:
                if decision.delay_ms / 1_000.0 > deadline_at - self.clock():
                    raise ProviderError(
                        failure(
                            FailureClass.TOTAL_REQUEST_TIMEOUT,
                            "retry delay exceeds total deadline",
                            attempt=attempt,
                        )
                    )
        ...
        raise ProviderError(
            failure(
                FailureClass.RETRY_EXHAUSTED,
                "provider retry budget exhausted",
                attempt=limits.provider_attempt_count,
            )
        )

    def _one_attempt(
        self, endpoint: ValidatedEndpoint, body: bytes, limits: Limits, deadline: float
    ) -> tuple[bytes, str | None, int, int | None]:
        remaining = deadline - self.clock()
        timeout = min(remaining, limits.provider_connect_timeout_ms / 1_000.0)
        if timeout <= 0:
            raise ProviderError(
                failure(FailureClass.CONNECT_TIMEOUT, "provider connect deadline expired")
            )
        ...
        try:
            ...
            if isinstance(status, bool) or not isinstance(status, int) or not 100 <= status <= 599:
                raise ProviderError(
                    failure(
                        FailureClass.MALFORMED_HTTP_RESPONSE, "provider HTTP status is malformed"
                    )
                )
            ...
            if declared is not None:
                if not isinstance(declared, str) or not re.fullmatch(r"[0-9]+", declared):
                    raise ProviderError(
                        failure(FailureClass.TRUNCATED_RESPONSE, "provider length is malformed")
                    )
                declared_size = int(declared)
                if declared_size < 0 or declared_size > limits.provider_response_bytes:
                    raise ProviderError(
                        failure(
                            FailureClass.OVERSIZED_RESPONSE, "provider response exceeds byte limit"
                        )
                    )
            chunks = bytearray()
            while True:
                ...
                try:
                    chunk = response.read(
                        min(8_192, limits.provider_response_bytes - len(chunks) + 1)
                    )
                except http.client.IncompleteRead:
                    raise ProviderError(
                        failure(FailureClass.TRUNCATED_RESPONSE, "provider response ended early")
                    ) from None
                if not isinstance(chunk, bytes):
                    raise ProviderError(
                        failure(
                            FailureClass.MALFORMED_HTTP_RESPONSE,
                            "provider response body is malformed",
                        )
                    )
                if not chunk:
                    break
            ...
            if declared_size is not None and len(chunks) != declared_size:
                raise ProviderError(
                    failure(
                        ...
                        "provider response ended before declared length",
                    )
                )
    ...
    def _set_read_timeout(
        ...
        remaining = min(
            deadline - self.clock(), limits.provider_read_inactivity_timeout_ms / 1_000.0
        )
        if remaining <= 0:
            raise ProviderError(
                failure(FailureClass.TOTAL_REQUEST_TIMEOUT, "provider total deadline expired")
            )
...
def _retry_after(value: str | None, limits: Limits) -> int | None:
    ...
    if (
        ...
        or len(value) > 16
        or not _RETRY_AFTER.fullmatch(value)
    ):
        return None
