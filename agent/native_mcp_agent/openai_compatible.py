...
from .endpoint_policy import (
    ValidatedEndpoint,
    redirect_rejection,
    resolve_production_transport_endpoint,
    validate_fake_loopback_endpoint,
    validate_loopback_transport_endpoint,
    validate_production_endpoint,
)
from .errors import FailureClass, ProviderError, failure, http_failure
...
class SyntheticFixture(Enum):
    """Closed project-owned content permitted by OpenAI-compatible adapter synthetic-only egress.
    ...
    TEST_PROMPT = (MessageRole.USER, "synthetic-only prompt")
    MANUAL_SMOKE_PROMPT = (MessageRole.USER, "Return a short synthetic acknowledgement.")
...
def _synthetic_message_authority() -> tuple[
    ...
    committed = {
        id(SyntheticFixture.TEST_PROMPT): _SyntheticFixtureSpec(
            SyntheticFixture.TEST_PROMPT,
            (MessageRole.USER, "synthetic-only prompt"),
            MessageRole.USER,
            "user",
            "synthetic-only prompt",
        ),
        id(SyntheticFixture.MANUAL_SMOKE_PROMPT): _SyntheticFixtureSpec(
            SyntheticFixture.MANUAL_SMOKE_PROMPT,
    ...
    issued: dict[
        int, tuple[weakref.ReferenceType[_AuthorizedSyntheticMessage], _SyntheticFixtureSpec]
    ] = {}
    ...
    def committed_fixture(fixture: SyntheticFixture) -> _SyntheticFixtureSpec:
        if type(fixture) is not SyntheticFixture:
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_AUTHORIZATION_FAILURE,
                    "synthetic fixture is not project-authorized",
                )
            )
        spec = committed.get(id(fixture))
        if (
            spec is None
            or fixture is not spec.fixture
            or fixture.value != spec.expected_fixture_value
        ):
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_AUTHORIZATION_FAILURE,
                    "synthetic fixture was changed or is not project-authorized",
                )
            )
        return spec
...
def _closed_config(value: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if (
        ...
        or {"endpoint", "model", "credentialEnv"} - set(value)
    ):
        raise ProviderError(
            failure(
                FailureClass.INVALID_PROVIDER_CONFIGURATION, "adapter configuration is not closed"
            )
        )
    return value
...
class OpenAICompatibleConfig:
    ...
    def __post_init__(self) -> None:
        try:
            ...
            raise ProviderError(
                failure(FailureClass.INVALID_PROVIDER_CONFIGURATION, "configured model is invalid")
            ) from error
        if (
            not isinstance(self.endpoint, str)
            or not self.endpoint
            or len(self.endpoint.encode("utf-8", "strict")) > 2_048
        ):
            raise ProviderError(
                failure(
                    FailureClass.INVALID_PROVIDER_CONFIGURATION, "configured endpoint is invalid"
                )
            )
        if not isinstance(self.credential_env, str) or not _CREDENTIAL_ENV.fullmatch(
            self.credential_env
        ):
            raise ProviderError(
                failure(FailureClass.INVALID_PROVIDER_CONFIGURATION, "credential source is invalid")
            )
        if self.verify_tls is not True:
            raise ProviderError(
                failure(FailureClass.TLS_VERIFICATION_FAILURE, "TLS verification is mandatory")
            )
        if (
            not isinstance(self.allow_loopback_http, bool)
            or self.data_flow != "synthetic-only"
            or not isinstance(self.limits, Limits)
        ):
            raise ProviderError(
                failure(
                    FailureClass.INVALID_PROVIDER_CONFIGURATION, "adapter configuration is invalid"
                )
            )
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> OpenAICompatibleConfig:
        ...
        if "limits" in raw:
            ...
            if not isinstance(supplied, Mapping) or set(supplied) - names:
                raise ProviderError(
                    failure(
                        FailureClass.INVALID_PROVIDER_CONFIGURATION, "adapter limits are not closed"
                    )
                )
            try:
                ...
                raise ProviderError(
                    failure(
                        FailureClass.INVALID_PROVIDER_CONFIGURATION, "adapter limits are invalid"
                    )
                ) from None
        return cls(
            ...
            verify_tls=raw.get("verifyTls", True),
            allow_loopback_http=raw.get("allowLoopbackHttp", False),
            data_flow=raw.get("dataFlow", "synthetic-only"),
            limits=limits,
        )
...
def openai_request_bytes(request: ProviderRequest, config: OpenAICompatibleConfig) -> bytes:
    ...
    if not isinstance(request, ProviderRequest) or request.model != config.model:
        raise ProviderError(
            failure(
                ...
                "provider request model does not match configured model",
            )
        )
    # OpenAI-compatible adapter has only a synthetic-only egress mode.  The authorization is
    ...
    for message in request.messages:
        ...
        if fixture is None:
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_AUTHORIZATION_FAILURE,
                    "synthetic-only request is not project-authorized",
                )
            )
    ...
    value: dict[str, Any] = {
        ...
        "messages": [
            {"role": item.wire_role, "content": item.content} for item in authorized_messages
        ],
        "tools": tools,
    ...
    if len(encoded) > config.limits.provider_request_bytes:
        raise ProviderError(
            failure(FailureClass.REQUEST_TOO_LARGE, "mapped request exceeds byte limit")
        )
    return encoded


def _openai_closed_object(
    ...
    try:
        ...
        raise ProviderError(
            failure(
                ...
                "OpenAI-compatible response shape is unsupported",
            )
        ) from error
...
def parse_openai_compatible_response(
    ...
    *,
    ...
    if not isinstance(choices, list) or len(choices) != 1:
        raise ProviderError(
            failure(FailureClass.UNSUPPORTED_PROVIDER_CONTENT, "OpenAI choices are unsupported")
        )
    choice = _openai_closed_object(
        choices[0], {"index", "message", "finish_reason", "logprobs"}, {"message"}, "OpenAI choice"
    )
    message = _openai_closed_object(
        choice["message"], {"role", "content", "tool_calls", "refusal"}, {"role"}, "OpenAI message"
    )
    if message["role"] != "assistant":
        raise ProviderError(
            failure(FailureClass.UNSUPPORTED_PROVIDER_CONTENT, "OpenAI message role is unsupported")
        )
    ...
    if has_text and has_calls:
        raise ProviderError(
            failure(
                FailureClass.UNSUPPORTED_PROVIDER_CONTENT,
                "mixed final text and tool calls are unsupported",
            )
        )
    if has_text:
        content = _bounded_text(
            message["content"],
            "OpenAI final content",
    ...
    if not has_calls or not isinstance(message["tool_calls"], list) or not message["tool_calls"]:
        raise ProviderError(
            failure(FailureClass.UNSUPPORTED_PROVIDER_CONTENT, "OpenAI content is unsupported")
        )
    calls: list[dict[str, str]] = []
    for item in message["tool_calls"]:
        call = _openai_closed_object(
            item, {"id", "type", "function"}, {"id", "type", "function"}, "OpenAI tool call"
        )
        if call["type"] != "function":
            raise ProviderError(
                failure(
                    FailureClass.UNSUPPORTED_PROVIDER_CONTENT, "OpenAI tool type is unsupported"
                )
            )
        function = _openai_closed_object(
            call["function"], {"name", "arguments"}, {"name", "arguments"}, "OpenAI function"
        )
        if (
            ...
            raise ProviderError(
                failure(FailureClass.INVALID_TOOL_PROPOSAL, "OpenAI tool call is malformed")
            )
        calls.append(
    ...
    neutral = json.dumps(
        {"toolCalls": calls}, ensure_ascii=True, sort_keys=True, separators=(",", ":")
    ).encode("utf-8")
...
class OpenAICompatibleTransport:
    ...
    def send_production(
        ...
        *,
        ...
        if (
            ...
            or endpoint.loopback_only
            or endpoint.scheme != "https"
        ):
            raise ProviderError(
                failure(
                    FailureClass.ENDPOINT_POLICY_REJECTION,
                    "credential transport requires production HTTPS",
                )
            )
        if not isinstance(credential, str) or not credential:
            raise ProviderError(
                failure(FailureClass.CREDENTIAL_UNAVAILABLE, "credential is unavailable")
            )
        return self._send(
            endpoint,
            body,
        ...
        *,
        ...
        if not isinstance(endpoint, ValidatedEndpoint) or endpoint.loopback_only is not True:
            raise ProviderError(
                failure(
                    FailureClass.ENDPOINT_POLICY_REJECTION,
                    "credential-free transport requires loopback HTTP",
                )
            )
        validate_loopback_transport_endpoint(endpoint)
        return self._send(
            endpoint, body, None, limits=limits, correlation_id=correlation_id, deadline=deadline
        )
    ...
    def _send(
        ...
        *,
        ...
        if not isinstance(body, bytes) or len(body) > limits.provider_request_bytes:
            raise ProviderError(
                failure(FailureClass.REQUEST_TOO_LARGE, "request exceeds byte limit")
            )
        if not isinstance(correlation_id, str) or not correlation_id.startswith("req-"):
            raise ProviderError(
                failure(FailureClass.LOCAL_VALIDATION_FAILURE, "correlation is invalid")
            )
        if not isinstance(endpoint, ValidatedEndpoint):
            raise ProviderError(
                failure(FailureClass.ENDPOINT_POLICY_REJECTION, "provider endpoint is invalid")
            )
        if endpoint.loopback_only:
            ...
            if credential is not None:
                raise ProviderError(
                    failure(
                        FailureClass.LOCAL_POLICY_FAILURE, "loopback transport forbids credentials"
                    )
                )
        ...
        if deadline is not None:
            if (
                ...
                raise ProviderError(
                    failure(FailureClass.TOTAL_REQUEST_TIMEOUT, "provider deadline is invalid")
                )
        ...
        for attempt in range(1, limits.provider_attempt_count + 1):
            ...
            try:
                ...
                attempt_endpoint = (
                    endpoint
                    if endpoint.loopback_only
                    else resolve_production_transport_endpoint(endpoint)
                )
                response, content_type, status, retry_after = self._one_attempt(
                    attempt_endpoint, body, credential, correlation_id, limits, deadline_at
                )
                if 300 <= status < 400:
                ...
                if (
                    ...
                    or content_type.lower().split(";", 1)[0].strip() != _JSON_CONTENT_TYPE
                ):
                    raise ProviderError(
                        failure(
                            FailureClass.INVALID_CONTENT_TYPE, "provider content type is invalid"
                        )
                    )
                return response
            except ProviderError as error:
                ...
                last = failure(
                    FailureClass.TLS_VERIFICATION_FAILURE, "certificate verification failed"
                )
                ...
                last = failure(
                    FailureClass.TOTAL_REQUEST_TIMEOUT
                    if self.clock() >= deadline_at
                    else FailureClass.READ_TIMEOUT,
                    "response timed out",
                )
                ...
                last = failure(
                    FailureClass.MALFORMED_HTTP_RESPONSE, "provider HTTP response is malformed"
                )
            last = replace(last, attempt=attempt)
            remaining_ms = max(0, int((deadline_at - self.clock()) * 1000))
            decision = decide_retry(
                last, completed_attempts=attempt, remaining_ms=remaining_ms, limits=limits
            )
            if not decision.eligible:
                if (
                    last.retryable
                    and attempt >= limits.provider_attempt_count
                    and limits.provider_attempt_count > 1
                ):
                    raise ProviderError(
                        failure(
                            FailureClass.RETRY_EXHAUSTED, "retry budget exhausted", attempt=attempt
                        )
                    )
    ...
    def _one_attempt(
        ...
        limits: Limits,
        deadline: float,
    ) -> tuple[bytes, str | None, int, int | None]:
        timeout = min(deadline - self.clock(), limits.provider_connect_timeout_ms / 1000.0)
        if timeout <= 0:
            raise ProviderError(
                failure(FailureClass.CONNECT_TIMEOUT, "connection deadline expired")
            )
        ...
        if endpoint.loopback_only:
            ...
            connection = _VerifiedHTTPSConnection(
                endpoint.host, endpoint.connect_host, endpoint.port, timeout
            )
        try:
            try:
                connection.connect()
                ...
                raise ProviderError(
                    failure(FailureClass.CONNECT_TIMEOUT, "connection timed out")
                ) from None
            headers = {
            ...
            if isinstance(status, bool) or not isinstance(status, int) or not 100 <= status <= 599:
                raise ProviderError(
                    failure(FailureClass.MALFORMED_HTTP_RESPONSE, "HTTP status is malformed")
                )
            content_type, retry_after = (
                response.getheader("Content-Type"),
                _retry_after(response.getheader("Retry-After"), limits),
            )
            if 300 <= status < 400:
            ...
            if declared is not None:
                if not isinstance(declared, str) or not re.fullmatch(r"[0-9]+", declared):
                    raise ProviderError(
                        failure(FailureClass.TRUNCATED_RESPONSE, "response length is malformed")
                    )
                if int(declared) > limits.provider_response_bytes:
                    raise ProviderError(
                        failure(FailureClass.OVERSIZED_RESPONSE, "response is oversized")
                    )
            chunks = bytearray()
            while True:
                self._set_read_timeout(connection, deadline, limits)
                ...
                if not isinstance(chunk, bytes):
                    raise ProviderError(
                        failure(FailureClass.MALFORMED_HTTP_RESPONSE, "response body is malformed")
                    )
                if not chunk:
                    break
                chunks.extend(chunk)
            ...
            if declared is not None and len(chunks) != int(declared):
                raise ProviderError(
                    failure(FailureClass.TRUNCATED_RESPONSE, "response is truncated")
                )
    ...
    def _set_read_timeout(
        self, connection: http.client.HTTPConnection, deadline: float, limits: Limits
    ) -> None:
        remaining = min(
            deadline - self.clock(), limits.provider_read_inactivity_timeout_ms / 1000.0
        )
        if remaining <= 0:
            raise ProviderError(
                failure(FailureClass.TOTAL_REQUEST_TIMEOUT, "provider deadline expired")
            )
...
class OpenAICompatibleProvider(BoundedProvider):
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.config, OpenAICompatibleConfig):
            raise ProviderError(
                failure(FailureClass.INVALID_PROVIDER_CONFIGURATION, "adapter config is invalid")
            )
        if self.transport is None:
            self.transport = OpenAICompatibleTransport()
        if not isinstance(self.transport, OpenAICompatibleTransport):
            raise ProviderError(
                failure(FailureClass.LOCAL_POLICY_FAILURE, "adapter transport is not project-owned")
            )
    ...
    def turn(
        ...
        *,
        ...
        if evidence:
            raise ProviderError(
                failure(
                    FailureClass.LOCAL_POLICY_FAILURE, "synthetic-only provider rejects evidence"
                )
            )
        ...
        if endpoint.loopback_only:
            ...
            raw = self.transport.send_loopback(
                endpoint,
                body,
            ...
            raw = self.transport.send_production(
                endpoint,
                body,
                credential,
                limits=self.config.limits,
        ...
        return parse_openai_compatible_response(
            raw, advertised_tools=request.tools, limits=self.config.limits
        )
