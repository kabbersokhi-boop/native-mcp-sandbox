#!/usr/bin/env python3
"""Offline deterministic tests for the OpenAI-compatible adapter OpenAI-compatible adapter."""
...
from agent.native_mcp_agent.errors import FailureClass, ProviderError  # noqa: E402
...
from scripts.openai_adapter_smoke import build_synthetic_smoke_request  # noqa: E402
...
TOOLS = (
    ...
    AdvertisedTool(
        ...
        {
            "type": "object",
            "properties": {"lines": {"type": "integer", "minimum": 1, "maximum": 9}},
            "required": ["lines"],
            "additionalProperties": False,
        },
        "tail",
...
SENTINEL = "PROVIDER_CREDENTIAL_SENTINEL_NOT_LEAKED"


def request() -> ProviderRequest:
    return ProviderRequest(
        "synthetic-model",
        (synthetic_fixture_message(SyntheticFixture.TEST_PROMPT),),
        TOOLS,
        32,
        RequestCorrelationId("req-10-4"),
    )
...
def config(endpoint: str, **changes: object) -> OpenAICompatibleConfig:
    values: dict[str, object] = {
        ...
        "credential_env": "NATIVE_MCP_TEST_TOKEN",
...
class ConfigAndMappingTests(unittest.TestCase):
    def test_closed_config_and_no_eager_credential_load(self) -> None:
        os.environ["NATIVE_MCP_TEST_TOKEN"] = SENTINEL
        cfg = OpenAICompatibleConfig.from_mapping(
            {
                "endpoint": "https://provider.example/v1/chat/completions",
                "model": "operator/model",
                "credentialEnv": "NATIVE_MCP_TEST_TOKEN",
                "limits": {"provider_attempt_count": 2},
            }
        )
    ...
    def test_request_schema_model_tools_stream_and_exact_byte_limit(self) -> None:
        ...
        exact = replace(
            cfg, limits=replace(DEFAULT_LIMITS, provider_request_bytes=len(raw))
        )
        ...
        with self.assertRaises(ProviderError) as raised:
            openai_request_bytes(
                ...
                replace(
                    cfg,
                    limits=replace(DEFAULT_LIMITS, provider_request_bytes=len(raw) - 1),
                ),
            )
        self.assertEqual(
    ...
    def test_synthetic_only_rejects_unmarked_content_and_closed_fixture_rejects_arbitrary_strings(
        self,
    ) -> None:
        cfg = config("http://127.0.0.1:1/v1/chat/completions")
        authorized = synthetic_fixture_message(SyntheticFixture.TEST_PROMPT)
        for message in (
            ProviderMessage(MessageRole.USER, "arbitrary host evidence"),
            replace(authorized),
        ):
            with self.subTest(message_type=type(message).__name__):
                unmarked = ProviderRequest(
                    "synthetic-model",
                    (message,),
                    TOOLS,
                    32,
        ...
        for forged_fixture in (
            "arbitrary host evidence",
            (MessageRole.USER, "arbitrary host evidence"),
        ):
            with (
                self.subTest(forged_fixture=forged_fixture),
    ...
    def test_synthetic_authorization_rejects_copies_and_message_or_fixture_mutation(
        self,
    ) -> None:
        cfg = config("http://127.0.0.1:1/v1/chat/completions")
        content_tampered = synthetic_fixture_message(SyntheticFixture.TEST_PROMPT)
        object.__setattr__(content_tampered, "content", "arbitrary host evidence")
        role_tampered = synthetic_fixture_message(SyntheticFixture.TEST_PROMPT)
        object.__setattr__(role_tampered, "role", MessageRole.ASSISTANT)
        copied = copy.copy(synthetic_fixture_message(SyntheticFixture.TEST_PROMPT))
        ...
        try:
            ...
            with self.assertRaises(ProviderError) as issued_raised:
                openai_request_bytes(forged, cfg)
            self.assertEqual(
                issued_raised.exception.failure.classification,
                FailureClass.LOCAL_AUTHORIZATION_FAILURE,
            )
        finally:
            object.__setattr__(fixture, "_value_", original_value)

    def test_authorized_synthetic_content_succeeds_with_deterministic_serialization(
        self,
    ) -> None:
        cfg = config("http://127.0.0.1:1/v1/chat/completions")
        first = request()
        second = request()
        self.assertEqual(
            openai_request_bytes(first, cfg), openai_request_bytes(second, cfg)
        )
        self.assertEqual(
            json.loads(openai_request_bytes(first, cfg))["messages"],
            [{"role": "user", "content": "synthetic-only prompt"}],
        )
    ...
    def test_manual_synthetic_smoke_uses_authorized_egress_path(self) -> None:
        ...
        self.assertIn(
            b"Return a short synthetic acknowledgement.",
            openai_request_bytes(smoke_request, cfg),
        )

    def test_endpoint_policy_rejects_http_production_userinfo_fragment_and_disabled_tls(
        self,
    ) -> None:
        for endpoint, kwargs, kind in (
            ...
            (
                ...
                {"verify_tls": False, "allow_loopback_http": False},
...
class ResponseParsingTests(unittest.TestCase):
    ...
    def test_final_one_and_multiple_calls(self) -> None:
        ...
        one = self.parse(
            b'{"choices":[{"message":{"role":"assistant","tool_calls":[{"id":"call-1","type":"function","function":{"name":"logs.search","arguments":"{\\"query\\":\\"x\\"}"}}]}}]}'
        )
        multiple = self.parse(
            b'{"choices":[{"message":{"role":"assistant","tool_calls":[{"id":"call-1","type":"function","function":{"name":"logs.search","arguments":"{\\"query\\":\\"x\\"}"}},{"id":"call-2","type":"function","function":{"name":"logs.tail","arguments":"{\\"lines\\":3}"}}]}}]}'
        )
    ...
    def test_parser_rejects_bad_and_boundary_response(self) -> None:
        ...
        for raw in (
            b"{",
            b'{"choices":[],"choices":[]}',
            b'{"choices":[{"message":{"role":"assistant","content":"x","unknown":1}}]}',
        ...
        self.assertEqual(
            parse_openai_compatible_response(
                good, advertised_tools=TOOLS, limits=exact
            ).message.content,
            "done",
        )
        with self.assertRaises(ProviderError) as raised:
            parse_openai_compatible_response(
                ...
                limits=replace(exact, provider_response_bytes=len(good) - 1),
            )
        self.assertEqual(
...
class TransportAndAuthorityTests(unittest.TestCase):
    ...
    def _provider(
        self, server: FakeProviderServer, **changes: object
    ) -> OpenAICompatibleProvider:
    ...
    def test_loopback_request_is_credential_free_and_retry_is_stable(self) -> None:
        with FakeProviderServer(
            FakeCase.RETRY_SUCCESS, openai_compatible=True
        ) as server:
            result = self._provider(server).turn(
                request(), (), timeout_ms=1_000, cancellation=None
            )
            ...
            self.assertEqual(
                [item["x-request-id"] for item in server.request_headers],
                ["req-10-4", "req-10-4"],
            )
            self.assertTrue(
                all("authorization" not in item for item in server.request_headers)
            )
            self.assertTrue(
                all(SENTINEL not in str(item) for item in server.request_headers)
            )
            surfaces = (
                repr(result),
                str(server.request_bodies),
                repr(self._provider(server).config),
            )
        ...
        class NoCredentialEnvironment:
            def get(self, _name: object, _default: object = None) -> object:
                raise AssertionError(
                    "loopback HTTP must not read configured credentials"
                )

        with FakeProviderServer(FakeCase.FINAL, openai_compatible=True) as server:
            provider = self._provider(server)
            with patch(
                "agent.native_mcp_agent.openai_compatible.os.environ",
                NoCredentialEnvironment(),
            ):
    ...
    def test_production_transport_is_the_only_credential_bearing_path(self) -> None:
            ...
            def send_production(
                ...
                **kwargs: object,
                ...
                if endpoint.scheme != "https" or kwargs["correlation_id"] != "req-10-4":
                    raise AssertionError(
                        "production credential was not confined to verified HTTPS"
                    )
        ...
        loopback = ValidatedEndpoint(
            "http://127.0.0.1:1/v1", "http", "127.0.0.1", "127.0.0.1", 1, "/v1", True
        )
        with self.assertRaises(ProviderError) as raised:
            OpenAICompatibleTransport().send_production(
                loopback,
                b"{}",
                SENTINEL,
    ...
    def test_verified_https_serializes_credential_only_as_authorization_header(
        self,
    ) -> None:
        ...
        with (
            patch(
                "agent.native_mcp_agent.openai_compatible._VerifiedHTTPSConnection",
                return_value=connection,
            ),
            patch(
                "agent.native_mcp_agent.openai_compatible.resolve_production_transport_endpoint",
    ...
    def test_synthetic_only_rejects_later_mcp_evidence_before_transmission(
        self,
    ) -> None:
        with FakeProviderServer(FakeCase.FINAL, openai_compatible=True) as server:
            with self.assertRaises(ProviderError) as raised:
                self._provider(server).turn(
                    request(), (object(),), timeout_ms=1_000, cancellation=None
                )  # type: ignore[arg-type]
            self.assertEqual(
                raised.exception.failure.classification,
                FailureClass.LOCAL_POLICY_FAILURE,
            )
            self.assertEqual(server.request_count, 0)

    def test_status_redirect_content_and_timeouts_are_classified(self) -> None:
        expected = {
            FakeCase.STATUS_400: FailureClass.HTTP_400_INVALID_REQUEST,
            FakeCase.STATUS_401: FailureClass.HTTP_401_AUTHENTICATION_FAILURE,
            FakeCase.STATUS_403: FailureClass.HTTP_403_AUTHORIZATION_FAILURE,
            FakeCase.STATUS_404: FailureClass.HTTP_404_ENDPOINT_OR_MODEL_NOT_FOUND,
            FakeCase.STATUS_408: FailureClass.HTTP_408_REQUEST_TIMEOUT,
            FakeCase.STATUS_413: FailureClass.HTTP_413_PAYLOAD_TOO_LARGE,
            FakeCase.STATUS_422: FailureClass.HTTP_422_SEMANTIC_REJECTION,
            FakeCase.STATUS_429: FailureClass.HTTP_429_RATE_LIMITED,
            FakeCase.STATUS_409: FailureClass.OTHER_PERMANENT_4XX,
            FakeCase.STATUS_500: FailureClass.TRANSIENT_5XX,
        ...
        for case, kind in expected.items():
            with (
                ...
                FakeProviderServer(case, openai_compatible=True) as server,
            ):
                with self.assertRaises(ProviderError) as raised:
                    self._provider(
                        server, limits=replace(DEFAULT_LIMITS, provider_attempt_count=1)
                    ).turn(request(), (), timeout_ms=1_000, cancellation=None)
                self.assertEqual(raised.exception.failure.classification, kind)
        for case, limits, kind in (
            (
                ...
                replace(
                    DEFAULT_LIMITS,
                    provider_read_inactivity_timeout_ms=10,
                    provider_total_timeout_ms=500,
                ),
            ...
            (
                ...
                replace(
                    ...
                    provider_total_timeout_ms=10,
                ),
                FailureClass.TOTAL_REQUEST_TIMEOUT,
            ),
        ):
            with (
                self.subTest(case=case, kind=kind),
                FakeProviderServer(case, openai_compatible=True) as server,
            ):
                with self.assertRaises(ProviderError) as raised:
                    self._provider(server, limits=limits).turn(
                        request(), (), timeout_ms=1_000, cancellation=None
                    )
                self.assertEqual(raised.exception.failure.classification, kind)

    def test_missing_credential_no_eager_load_tls_and_authority_marker(self) -> None:
        os.environ.pop("NATIVE_MCP_TEST_TOKEN", None)
        cfg = config(
            "https://provider.example/v1/chat/completions", allow_loopback_http=False
        )
        provider = OpenAICompatibleProvider(cfg)
        with self.assertRaises(ProviderError) as raised:
            provider.turn(request(), (), timeout_ms=10, cancellation=None)
        self.assertEqual(
            raised.exception.failure.classification, FailureClass.CREDENTIAL_UNAVAILABLE
        )
        ...
        self.assertEqual(
            tls.exception.failure.classification, FailureClass.TLS_VERIFICATION_FAILURE
        )
