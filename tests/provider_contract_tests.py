#!/usr/bin/env python3
"""Deterministic provider contracts contract, policy, transport, and secrecy tests."""
...
def request(*, message: str = "synthetic", limits=DEFAULT_LIMITS) -> ProviderRequest:
    ...
    return ProviderRequest(
        "synthetic-model",
        (ProviderMessage(MessageRole.USER, message),),
        TOOLS,
        128,
...
class ContractTests(unittest.TestCase):
    ...
    def test_closed_schema_rejections(self) -> None:
        ...
        expected = {
            ...
            FailureClass.DUPLICATE_KEY_JSON,
    ...
    def test_duplicate_json_and_limits(self) -> None:
        ...
        self.assertEqual(
            raised.exception.failure.classification, FailureClass.DUPLICATE_KEY_JSON
        )
        exact = replace(DEFAULT_LIMITS, provider_request_bytes=64)
        with self.assertRaises(ProviderError) as raised:
            request(message="x" * 500).to_json_bytes(exact)
        self.assertEqual(
            raised.exception.failure.classification, FailureClass.REQUEST_TOO_LARGE
        )
    ...
    def test_action_identity_is_provider_id_independent(self) -> None:
        first = ProviderToolCallProposal(
            ToolCallId("one"), "logs.search", {"query": "x"}
        )
        second = ProviderToolCallProposal(
            ToolCallId("two"), "logs.search", {"query": "x"}
        )
    ...
    def test_closed_tool_schema_subset(self) -> None:
        ...
        self.assertEqual(
            parse_provider_response(valid, advertised_tools=(nested_tool,))[0].name,
            "nested",
        )
        for arguments in (
            ...
            raw = json.dumps(
                {
                    "toolCalls": [
                        {"id": "call-1", "name": "nested", "arguments": arguments}
                    ]
                }
            ).encode()
        ...
        bad_schemas = (
            ...
            {
                "type": "object",
                "properties": {"x": {"type": "string", "unknown": 1}},
                "additionalProperties": False,
            },
            {
                "type": "object",
    ...
    def test_configured_limits_are_operational(self) -> None:
        ...
        req = ProviderRequest(
            "m", (message,), TOOLS, 128, RequestCorrelationId("req-10-1")
        )
        self.assertLessEqual(
            len(req.to_json_bytes(small)), small.provider_request_bytes
        )
        with self.assertRaises(ProviderError):
        ...
        self.assertEqual(
            len(
                parse_closed_json(
                    b'{"x":1}', replace(DEFAULT_LIMITS, provider_response_bytes=7)
                )
            ),
            1,
        )
        with self.assertRaises(ProviderError):
            parse_closed_json(
                b'{"x":1}', replace(DEFAULT_LIMITS, provider_response_bytes=6)
            )
        self.assertEqual(
            parse_closed_json(
                b'{"a":1}', replace(DEFAULT_LIMITS, object_array_items=1)
            ),
            {"a": 1},
        )
        with self.assertRaises(ProviderError):
        ...
        self.assertEqual(
            parse_closed_json(
                b'{"a":{"b":1}}', replace(DEFAULT_LIMITS, json_nesting_depth=2)
            ),
            {"a": {"b": 1}},
        )
        with self.assertRaises(ProviderError):
            parse_closed_json(
                b'{"a":{"b":1}}', replace(DEFAULT_LIMITS, json_nesting_depth=1)
            )
    ...
    def test_configured_collection_and_serialization_boundaries(self) -> None:
        ...
        request_with_messages = ProviderRequest(
            "m", messages, TOOLS, 128, RequestCorrelationId("req-10-1")
        )
        with self.assertRaises(ProviderError):
            request_with_messages.to_json_bytes(DEFAULT_LIMITS)
        self.assertTrue(
            request_with_messages.to_json_bytes(
                replace(DEFAULT_LIMITS, message_count=9, provider_request_bytes=8192)
            )
        )
        tools = tuple(tool(f"tool-{index}") for index in range(9))
        request_with_tools = ProviderRequest(
            "m",
            (ProviderMessage(MessageRole.USER, "x"),),
            tools,
            128,
            RequestCorrelationId("req-10-1"),
        )
        ...
        self.assertTrue(
            request_with_tools.to_json_bytes(
                replace(
                    DEFAULT_LIMITS, advertised_tool_count=9, provider_request_bytes=8192
                )
            )
        )
        ...
        self.assertEqual(
            request().to_json_bytes(
                replace(DEFAULT_LIMITS, provider_request_bytes=len(encoded_request))
            ),
            encoded_request,
        )
        with self.assertRaises(ProviderError) as raised:
            request().to_json_bytes(
                replace(DEFAULT_LIMITS, provider_request_bytes=len(encoded_request) - 1)
            )
        self.assertEqual(
            raised.exception.failure.classification, FailureClass.REQUEST_TOO_LARGE
        )
        described = AdvertisedTool(
            "described",
            {
                "type": "object",
                "properties": {},
        ...
        described_request = ProviderRequest(
            "m",
            (ProviderMessage(MessageRole.USER, "x"),),
            (described,),
            128,
        ...
        self.assertTrue(
            described_request.to_json_bytes(
                replace(
                    DEFAULT_LIMITS,
                    tool_definition_bytes=len(definition),
                    provider_request_bytes=8192,
                )
            )
        )
        with self.assertRaises(ProviderError):
            described_request.to_json_bytes(
                replace(
                    DEFAULT_LIMITS,
                    tool_definition_bytes=len(definition) - 1,
                    provider_request_bytes=8192,
                )
            )
        calls = [
            {"id": f"call-{index}", "name": "logs.search", "arguments": "{}"}
        ...
        self.assertEqual(
            len(
                parse_provider_response(
                    raw,
                    advertised_tools=TOOLS,
                    limits=replace(DEFAULT_LIMITS, proposed_tool_call_count=5),
                )
            ),
            5,
        )
        ...
        large_raw = json.dumps(
            {
                "toolCalls": [
                    {"id": "call-1", "name": "logs.search", "arguments": large_args}
                ]
            }
        ).encode()
        ...
        self.assertEqual(
            len(
                parse_provider_response(
                    large_raw,
                    advertised_tools=TOOLS,
                    limits=replace(DEFAULT_LIMITS, tool_argument_bytes=argument_size),
                )
            ),
            1,
        )
        with self.assertRaises(ProviderError):
            parse_provider_response(
                large_raw,
                advertised_tools=TOOLS,
                limits=replace(DEFAULT_LIMITS, tool_argument_bytes=argument_size - 1),
            )
        ...
        self.assertEqual(
            parse_transcript(
                transcript_bytes,
                replace(DEFAULT_LIMITS, transcript_bytes=len(transcript_bytes)),
            ).event,
            "event",
        )
        with self.assertRaises(ProviderError):
            parse_transcript(
                transcript_bytes,
                replace(DEFAULT_LIMITS, transcript_bytes=len(transcript_bytes) - 1),
            )
        self.assertTrue(
            decide_retry(
                http_failure(503),
                ...
                remaining_ms=500,
                limits=replace(DEFAULT_LIMITS, retry_backoff_ms=100),
            ).eligible
        )
        self.assertEqual(
            decide_retry(
        ...
        for raw in malformed:
            with self.subTest(raw=repr(raw)):
                ...
                self.assertIsInstance(
                    raised.exception.failure.classification, FailureClass
                )
...
class EndpointTests(unittest.TestCase):
    def test_production_endpoint_policy(self) -> None:
        self.assertEqual(
            validate_production_endpoint("https://provider.example/v1/chat").scheme,
            "https",
        )
        for url in (
        ...
        with self.assertRaises(ProviderError) as raised:
            validate_production_endpoint(
                "https://provider.example/v1", verify_tls=False
            )
        self.assertEqual(
            raised.exception.failure.classification,
    ...
    def test_fake_endpoint_is_explicit_and_loopback_only(self) -> None:
        ...
        self.assertTrue(
            validate_fake_loopback_endpoint(
                "http://localhost:1234/x", allow_loopback_http=True, resolver=loopback
            ).loopback_only
        )
        ...
        def mixed(_host, _port, _family, _type):
            return [
                ...
                (socket.AF_INET, socket.SOCK_STREAM, 6, "", ("10.0.0.1", 1)),
            ]

        with self.assertRaises(ProviderError):
            validate_fake_loopback_endpoint(
                "http://localhost:1234/x", allow_loopback_http=True, resolver=mixed
            )
        with self.assertRaises(ProviderError):
            validate_fake_loopback_endpoint(
                "http://localhost:1234/x", allow_loopback_http=False, resolver=loopback
            )
    ...
    def test_transport_revalidates_forged_authority_before_connection(self) -> None:
        ...
        for endpoint in (
            ...
            ValidatedEndpoint(
                "http://127.0.0.1:1234/x?secret=1",
                "http",
                "127.0.0.1",
                "127.0.0.1",
                1234,
...
class TransportTests(unittest.TestCase):
    ...
    def run_case(self, case: FakeCase, limits=DEFAULT_LIMITS):
        with FakeProviderServer(case) as provider:
            return self.transport(limits).send(
    ...
    def test_successful_bounded_requests(self) -> None:
        with FakeProviderServer(FakeCase.FINAL) as provider:
            result = self.transport().send(
                provider.validated_endpoint(), request(), correlation_id="req-10-1"
            )
    ...
    def test_response_failures_and_statuses(self) -> None:
        ...
        for case, classification in cases.items():
            with self.subTest(case=case):
                ...
                self.assertEqual(
                    raised.exception.failure.classification, classification
                )
        ...
        self.assertEqual(
            raised.exception.failure.classification, FailureClass.OTHER_PERMANENT_4XX
        )
        for case in (
            ...
            with self.subTest(case=case), self.assertRaises(ProviderError) as raised:
                self.run_case(case, one_attempt)
            self.assertEqual(
                raised.exception.failure.classification, FailureClass.REDIRECT_REJECTED
            )
            self.assertNotIn("provider.invalid", str(raised.exception))
        with self.assertRaises(ProviderError) as raised:
            self.run_case(
                FakeCase.RETRY_AFTER, replace(DEFAULT_LIMITS, provider_attempt_count=1)
            )
        self.assertEqual(
            raised.exception.failure.classification, FailureClass.HTTP_429_RATE_LIMITED
        )
        with self.assertRaises(ProviderError) as raised:
            self.run_case(
                FakeCase.EXCESSIVE_RETRY_AFTER,
                replace(DEFAULT_LIMITS, provider_attempt_count=1),
            )
        self.assertEqual(
            raised.exception.failure.classification, FailureClass.HTTP_429_RATE_LIMITED
        )
    ...
    def test_size_timeout_connection_and_retry(self) -> None:
        tiny = replace(
            DEFAULT_LIMITS, provider_response_bytes=128, provider_attempt_count=1
        )
        with self.assertRaises(ProviderError) as raised:
            self.run_case(FakeCase.OVERSIZED, tiny)
        self.assertEqual(
            raised.exception.failure.classification, FailureClass.OVERSIZED_RESPONSE
        )
        timed = replace(
            DEFAULT_LIMITS,
            provider_read_inactivity_timeout_ms=20,
            provider_total_timeout_ms=100,
            provider_attempt_count=1,
        )
        with self.assertRaises(ProviderError) as raised:
            self.run_case(FakeCase.READ_TIMEOUT, timed)
        self.assertEqual(
            raised.exception.failure.classification, FailureClass.READ_TIMEOUT
        )
        with self.assertRaises(ProviderError) as raised:
            self.run_case(
                FakeCase.CONNECTION_CLOSE,
                replace(DEFAULT_LIMITS, provider_attempt_count=1),
            )
        self.assertEqual(
            raised.exception.failure.classification,
        ...
        decision = decide_retry(
            http_failure(429, retry_after_ms=500),
            completed_attempts=1,
            remaining_ms=600,
        ...
        with self.assertRaises(ProviderError) as raised:
            self.run_case(
                FakeCase.STATUS_503, replace(DEFAULT_LIMITS, provider_attempt_count=2)
            )
        self.assertEqual(
            raised.exception.failure.classification, FailureClass.RETRY_EXHAUSTED
        )
        self.assertEqual(
            self.run_case(
                FakeCase.RETRY_SUCCESS,
                replace(DEFAULT_LIMITS, provider_attempt_count=2),
            ).message.content,
            "synthetic guidance",
        )
        self.assertEqual(
            self.run_case(
                ...
                replace(
                    DEFAULT_LIMITS,
                    provider_read_inactivity_timeout_ms=500,
                    provider_total_timeout_ms=1_000,
                    provider_attempt_count=1,
                ),
            ).message.content,
            "synthetic guidance",
        ...
        with self.assertRaises(ProviderError) as raised:
            self.run_case(
                ...
                replace(
                    DEFAULT_LIMITS,
                    provider_read_inactivity_timeout_ms=1_000,
                    provider_total_timeout_ms=50,
                    provider_attempt_count=1,
                ),
            )
        self.assertEqual(
            raised.exception.failure.classification, FailureClass.TOTAL_REQUEST_TIMEOUT
        )
    ...
    def test_request_too_large_is_rejected_before_transmission(self) -> None:
        ...
        with FakeProviderServer(FakeCase.FINAL) as provider:
            with self.assertRaises(ProviderError) as raised:
                self.transport(limits).send(
                    provider.validated_endpoint(),
                    request(message="x" * 200),
                    limits=limits,
                    correlation_id="req-10-1",
                )
            self.assertEqual(
    ...
    def test_configured_connect_timeout_reaches_connection_factory(self) -> None:
        ...
        limits = replace(
            DEFAULT_LIMITS,
            provider_connect_timeout_ms=77,
            provider_total_timeout_ms=1_000,
            provider_attempt_count=1,
        )
        with self.assertRaises(ProviderError) as raised:
            LoopbackFakeTransport(connection_factory=factory).send(
                endpoint, request(), limits=limits, correlation_id="req-10-1"
            )
        self.assertEqual(
            raised.exception.failure.classification, FailureClass.CONNECT_TIMEOUT
        )
...
class RedactionEnvironmentTranscriptTests(unittest.TestCase):
    def test_redaction_is_deterministic_idempotent_and_structural(self) -> None:
        secrets = (
            "api-sentinel-10-1",
            "bearer-sentinel-10-1",
            "proxy-sentinel-10-1",
            "exception-sentinel-10-1",
        )
        ...
        self.assertEqual(
            redact_headers(
                {"Authorization": "Bearer api-sentinel-10-1", "X-Id": "ok"}, secrets
            )["authorization"],
            REDACTED,
        )
        self.assertNotIn(
            "api-sentinel-10-1",
            json.dumps(
                redact_json({"api_key": secrets[0], "message": source}, secrets)
            ),
        )
        self.assertNotIn(
            "user:pass", redact_url("https://user:pass@provider.example/x")
        )
        self.assertNotIn(
            secrets[-1], redact_exception(RuntimeError(secrets[-1]), secrets)
        )
    ...
    def test_redaction_is_safe_at_utf8_and_overlap_boundaries(self) -> None:
        ...
        for source in (
            ...
            self.assertEqual(
                result, redact_text(result, (secret,), maximum=len(secret) + 3)
            )
        ...
        self.assertEqual(
            redact_json(
                {"path": "/absolute/secret", "diagnostic": "pid=424242", "count": 42}
            ),
            {"path": REDACTED, "diagnostic": REDACTED, "count": 42},
        )
    ...
    def test_environment_is_new_minimal_mapping(self) -> None:
        parent = {
            "LANG": "C.UTF-8",
            "PATH": "/usr/bin",
            "OPENAI_API_KEY": "api-sentinel-10-1",
            "HTTPS_PROXY": "proxy-sentinel-10-1",
        }
        ...
        with self.assertRaises(ProviderError):
            build_child_environment(
                {"HTTP_PROXY": "http://127.0.0.1:1"}, ("HTTP_PROXY",), allow_proxy=True
            )
    ...
    def test_transcript_canonical_and_closed(self) -> None:
        event = TranscriptEvent(
            ...
            "synthetic-model",
            RequestCorrelationId("req-10-1"),
            EvidenceProvenance.PROVIDER_SUGGESTION,
            metadata={"mode": "synthetic-only"},
        )
    ...
    def test_transcript_parser_bounds_types_and_failures(self) -> None:
        ...
        bad_values = [
            {**base, "metadata": []},
            {**base, "metadata": {"mode": 1}},
            {**base, "retryEligible": 1},
            {**base, "byteCount": True},
            {**base, "byteCount": -1},
            {**base, "failureClass": "not-a-failure"},
            {**base, "provenance": "not-provenance"},
            {**base, "proposalIds": ["call-1", "call-1"]},
            {**base, "proposalIds": ["call/1"]},
            {**base, "event": "Authorization: Bearer secret"},
            {**base, "adapter": "/absolute/path"},
            {**base, "model": "api-key-sentinel"},
            {**base, "correlationId": "user:pass@host"},
        ]
        ...
        with self.assertRaises(ProviderError):
            parse_transcript(
                ...
                replace(DEFAULT_LIMITS, object_array_items=2),
            )
    ...
    def test_transcript_identity_fields_never_emit_sentinels(self) -> None:
        ...
        for sentinel in sentinels:
            with self.subTest(sentinel=sentinel):
                ...
                self.assertNotIn(
                    sentinel, json.dumps(raised.exception.failure.__dict__, default=str)
                )
    ...
    def test_representative_scenarios_are_byte_identical_twice(self) -> None:
        ...
        for _ in range(2):
            event = TranscriptEvent(
                ...
                RequestCorrelationId("req-10-1"),
                EvidenceProvenance.LOCAL_CONTROL_EVENT,
                failure_class=FailureClass.HTTP_401_AUTHENTICATION_FAILURE,
