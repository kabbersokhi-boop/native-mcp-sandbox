...
from native_mcp_agent.contracts import (
    ProviderMessage,
    ProviderRequest,
...
from native_mcp_agent.mcp_orchestrator import (
    CancellationToken,
    McpStdioClient,
    Orchestrator,
    Deadline,
...
from native_mcp_agent.transcript import BoundedTranscript, parse_bounded_transcript
...
def client(case="normal", limits=DEFAULT_LIMITS):
    return McpStdioClient(
        ...
        parent_environment={"LANG": "C", "SECRET_SENTINEL": "x", "HTTP_PROXY": "bad"},
        limits=limits,
    )


def deadline(c, ms=1000):
    return Deadline(c.clock() + ms / 1000, c.clock)


def provider(*responses, delay_ms=0):
    return ScriptedProvider(tuple(responses), delay_ms=delay_ms)
...
class Tests(unittest.TestCase):
    def test_lifecycle_environment_and_cleanup(self):
        ...
        self.assertEqual(c.environment, {"LANG": "C"})
        ...
        self.assertTrue(
            {
                ...
                "shutdown_complete",
                "outcome",
            }.issubset({event["event"] for event in events})
        )
    ...
    def test_changed_surface_and_final_boundary(self):
        c = client("changing_tools")
        d = deadline(c)
        c.initialize_and_capture(d)
        with self.assertRaises(ProviderError):
            c.revalidate_surface(d)
        c.close(d, suppress=True)

    def test_direct_forged_authorization_never_writes(self):
        c = client()
        d = deadline(c)
        s = c.initialize_and_capture(d)
        before = c.next_id
        bad = AuthorizedMcpAction(
            s.identity, proposal("call-1").action_identity, "call-1", "nope", {}, b"{}"
        )
        with self.assertRaises(ProviderError):
            c.execute(bad, d)
    ...
    def test_serial_evidence_and_redaction(self):
        ...
        self.assertNotIn("SECRET_SENTINEL", str(out.evidence))
    ...
    def test_deadline_no_evidence_after_expiry(self):
        limits = replace(
            DEFAULT_LIMITS, orchestration_total_timeout_ms=100, mcp_call_timeout_ms=500
        )
        c = client("delay", limits)
        started = time.monotonic()
        out = Orchestrator(c, provider((proposal("call-1"),)), limits=limits).run(req())
        self.assertIn(out.outcome, {"failed", "deadline"})
    ...
    def test_scripted_provider_timeout_does_not_sleep_full_delay(self):
        ...
        started = time.monotonic()
        with self.assertRaises(ProviderError):
            provider.turn(req(), (), timeout_ms=50, cancellation=None)
    ...
    def test_result_contract_and_transcript_determinism(self):
        c = client("malformed_result")
        out = Orchestrator(c, provider((proposal("call-1"),))).run(req())
        self.assertEqual(out.outcome, "failed")
        a = (
            Orchestrator(
                client(),
...
class ShutdownBoundaryTests(unittest.TestCase):
    ...
    class Process:
        def __init__(self, clock, graceful):
            self.clock, self.graceful, self.stdin, self.stdout, self.stderr = (
                clock,
                graceful,
                ShutdownBoundaryTests.Stream(),
                None,
                None,
            )
            self.calls = []
    ...
    def _client_with_process(self, clock, graceful):
        c = client(limits=replace(DEFAULT_LIMITS, graceful_shutdown_timeout_ms=20))
        c.process = self.Process(clock, graceful)
        return c

    def test_graceful_shutdown_within_remaining_budget_reaps_and_clears_ownership(self):
        clock = self.Clock()
        c = self._client_with_process(clock, True)
        process = c.process
        result = c.close(Deadline(0.100, clock), suppress=True)
        waits = [item[1] for item in process.calls if item[0] == "wait"]
        self.assertEqual(result, "terminate")
        self.assertEqual(waits, [0.02])
        self.assertLessEqual(sum(waits), 0.10)
    ...
    def test_ignored_sigterm_fixture_uses_positive_remaining_budget_for_kill_reap(self):
        ...
        limits = replace(DEFAULT_LIMITS, graceful_shutdown_timeout_ms=20)
        c = client("ignore_shutdown", limits)
        d = deadline(c, 500)
        c.initialize_and_capture(d)
        process = InstrumentedProcess(c.process)
        c.process = process
        result = c.close(d, suppress=True)
        self.assertEqual(result, "kill")
        self.assertEqual(len(process.waits), 2)
        self.assertLessEqual(process.waits[0], 0.02)
        self.assertGreater(process.waits[1], 0)
        self.assertLessEqual(sum(process.waits), 0.5)
        self.assertIsNone(c.process)

    def test_expired_deadline_kills_without_new_reap_wait_and_reports_unreaped(self):
        clock = self.Clock()
        c = self._client_with_process(clock, False)
        process = c.process
        process.reap_after_kill = False
        result = c.close(Deadline(0.020, clock), suppress=True)
        waits = [item[1] for item in process.calls if item[0] == "wait"]
        self.assertEqual(result, "unreaped")
        self.assertEqual(waits, [0.02, 0])
...
class DeadlineTests(unittest.TestCase):
    def test_startup_readiness_limit_and_overall_limit(self):
        limits = replace(
            DEFAULT_LIMITS,
            process_startup_timeout_ms=40,
            mcp_initialize_timeout_ms=500,
            orchestration_total_timeout_ms=500,
        )
        c = client("delayed_start", limits)
        started = time.monotonic()
        out = Orchestrator(c, provider(), limits=limits).run(req())
        self.assertEqual(out.outcome, "deadline")
        self.assertLess(time.monotonic() - started, 0.15)
        self.assertIsNone(c.process)
        limits = replace(
            DEFAULT_LIMITS,
            process_startup_timeout_ms=500,
            mcp_initialize_timeout_ms=500,
            orchestration_total_timeout_ms=40,
        )
        c = client("delayed_start", limits)
        out = Orchestrator(c, provider(), limits=limits).run(req())
        self.assertEqual(out.outcome, "deadline")
    ...
    def test_initialize_and_tools_list_limits(self):
        for case, field in (
            ("delayed_initialize", "mcp_initialize_timeout_ms"),
            ("delayed_list", "mcp_tools_list_timeout_ms"),
        ):
            limits = replace(DEFAULT_LIMITS, **{field: 40})
    ...
    def test_provider_response_exact_and_one_over_bytes(self):
        ...
        for bound, expected in ((encoded, "final"), (encoded - 1, "failed")):
            limits = replace(DEFAULT_LIMITS, provider_response_bytes=bound)
            c = client(limits=limits)
            self.assertEqual(
                Orchestrator(c, provider(response), limits=limits).run(req()).outcome,
                expected,
            )
...
class AuthorizationBoundaryTests(unittest.TestCase):
    def test_every_invalid_final_action_does_not_write(self):
        ...
        invalid = (
            ("unadvertised", "nope", {}),
            ("unknown_field", "logs.search", {"other": "x"}),
            ("wrong_type", "logs.search", {"query": 1}),
            ("oversized", "logs.search", {"query": "x" * 20}),
            ("nesting", "logs.search", {"query": deep}),
            ("collection", "logs.search", {"query": ["x"] * 33}),
        )
        for _name, name, args in invalid:
            with self.assertRaises(ProviderError):
                c.authorize(aid, "call-1", name, args)
            self.assertEqual(c.next_id, before)
        good = c.authorize(aid, "call-1", "logs.search", {"query": "x"})
        for action in (
            replace(good, surface_identity="0" * 64),
            replace(good, argument_bytes=b"{}"),
            replace(good, action_id=proposal("call-2").action_identity),
            AuthorizedMcpAction(
                surface.identity,
                aid,
                "call-1",
    ...
    def test_authorized_snapshot_is_immutable_and_valid_action_writes_once(self):
        ...
        self.assertEqual(action.arguments["query"], "original")
        self.assertEqual(c.next_id, 4)
        c.close(d, suppress=True)


class CorrelationAndCancellationTests(unittest.TestCase):
    def test_unsolicited_response_fails_closed(self):
        self._assert_bad("unsolicited")

    def test_future_response_fails_closed(self):
        self._assert_bad("future_id")

    def test_duplicate_completed_response_fails_closed(self):
        self._assert_bad("duplicate_completed")

    def _assert_bad(self, case):
        c = client(case)
        out = Orchestrator(c, provider(), limits=c.limits).run(req())
        self.assertEqual(out.outcome, "failed")
        self.assertIsNone(c.process)
    ...
    def test_cancellation_during_provider_reaps_child(self):
        ...
        c = client()
        trigger = threading.Thread(target=lambda: (time.sleep(0.02), token.cancel()))
        trigger.start()
        out = Orchestrator(
            c,
            provider(
        ...
        self.assertIsNone(c.process)
        self.assertIn(b'"cancelled"', out.transcript)
    ...
    def test_cancellation_during_mcp_wait_skips_later_actions(self):
        ...
        token = CancellationToken()
        c = client("delay")
        trigger = threading.Thread(target=lambda: (time.sleep(0.05), token.cancel()))
        trigger.start()
        out = Orchestrator(
            c,
            provider((proposal("call-1", "a"), proposal("call-2", "b"))),
            cancellation=token,
        ).run(req())
        trigger.join()
        self.assertEqual(out.outcome, "cancelled")
...
class AtMostOnceTests(unittest.TestCase):
    def test_changed_content_reusing_call_id_in_one_response_is_rejected_before_write(
        self,
    ):
        c = client()
        out = Orchestrator(
...
class EvidenceAndTranscriptTests(unittest.TestCase):
    def test_closed_result_variants_fail(self):
        for case in (
            ...
            "result_wrong_content",
            "result_unknown_block",
            "result_unknown_type",
            "result_missing_text",
            "result_wrong_text",
    ...
    def test_serial_probe_reports_one_active_call_in_provider_order(self):
        ...
        self.assertEqual(len(out.execution_order), 2)
        self.assertNotEqual(out.execution_order[0], out.execution_order[1])
    ...
    def test_transcript_reserves_terminal_without_erasing_prefix(self):
        ...
        terminal = {
            "schemaVersion": 2,
            "events": [
                {"event": "process_start", "metadata": {}},
                {"event": "transcript_limit", "metadata": {}},
            ],
            "limited": True,
        }
        limit = len(
