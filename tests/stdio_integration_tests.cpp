...
void test_unconfigured_server(const std::string& executable) {
  ...
  const std::string expected =
      ...
      "{\"id\":2,\"jsonrpc\":\"2.0\",\"result\":{\"capabilities\":{\"tools\":{}},\"protocolVersion\":\"2025-11-25\",\"serverInfo\":{\"name\":\"native-mcp-sandbox\",\"version\":\"0.12.1\"}}}\n"
...
void test_configured_tools(const std::string& executable) {
  ...
  expect(initialized["result"]["serverInfo"]["version"] == "0.12.1",
         "configured initialize must report the planned v0.12.1 version");
  ...
  expect(tools.is_array() && tools.size() == 4U &&
         ...
         "configured server must advertise exactly four bounded tools");
