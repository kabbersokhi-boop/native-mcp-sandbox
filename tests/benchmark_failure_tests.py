...
def rejects(command: list[str], data: bytes = b"") -> None:
    try:
        run_benchmarks.bounded_run(command, data)
    except Exception:
        return
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="nms-benchmark-failure-") as directory:
        ...
        fake(timeout, "import time; time.sleep(20)")
        fake(flood, "import sys; sys.stdout.write('x'*300000)")
        fake(err, "import sys; sys.stderr.write('unexpected')")
        fake(nonzero, "raise SystemExit(3)")
        fake(malformed, "print('not-json')")
        rejects([str(timeout)])
        rejects([str(flood)])
        rejects([str(err)])
        rejects([str(nonzero)])
        try:
            run_benchmarks.validate_responses(b"not-json\n", False)
        except Exception:
            pass
        else:
            raise AssertionError("malformed JSON output accepted")
        stale = root / "stale.json"
        stale.write_text('{"complete":true}', encoding="utf-8")
        result = subprocess.run(
            [
                sys.executable,
                str(Path(__file__).resolve().parents[1] / "scripts/run_benchmarks.py"),
                "--benchmark",
                str(timeout),
                "--server",
                str(timeout),
                "--fixtures",
                str(Path(__file__).resolve().parents[1] / "benchmarks/fixtures"),
                "--output",
                str(stale),
            ],
            capture_output=True,
        ...
        if result.returncode == 0 or stale.exists():
            raise AssertionError("failed campaign left a complete report")
        valid_unconfigured = b'{"id":1,"result":{"protocolVersion":"2025-11-25","capabilities":{}}}\n{"id":2,"result":{"tools":[]}}\n'
        for bad in (
            valid_unconfigured.replace(b'"result"', b'"error"', 1),
            valid_unconfigured + valid_unconfigured.splitlines()[0] + b"\n",
            valid_unconfigured.splitlines()[0] + b"\n",
            valid_unconfigured.replace(b'"tools":[]', b'"tools":"bad"'),
        ):
            try:
                run_benchmarks.validate_responses(bad, False)
            except Exception:
                pass
            else:
                raise AssertionError("invalid response semantics accepted")
        metadata = {
            key: {"available": True, "value": "x"}
            for key in (
                "repositoryCommit",
                "dirtyWorktree",
        ...
        for mutation in mutations:
            ...
            candidate.pop(mutation.get("missing", "__none__"), None)
            candidate.update(
                {key: value for key, value in mutation.items() if key != "missing"}
            )
            try:
                validate(
                    candidate,
