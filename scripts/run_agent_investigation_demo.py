#!/usr/bin/env python3
"""Run the deterministic Phase 8 investigation demonstration."""
...
def tool_request(request_id: int, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
...
def build_protocol_input() -> tuple[str, list[dict[str, Any]], list[dict[str, Any]]]:
    initialize_params = {
        ...
        "capabilities": {},
        "clientInfo": {"name": "phase-8-demo", "version": VERSION},
    }
    tool_calls = [
        tool_request(10, "logs.search", {"root": ROOT_NAME, "path": LOG_NAME, "query": CORRELATION_ID, "caseSensitive": True, "maxMatches": 10}),
        tool_request(11, "logs.search", {"root": ROOT_NAME, "path": LOG_NAME, "query": "ERROR", "caseSensitive": True, "maxMatches": 10}),
        tool_request(12, "logs.tail", {"root": ROOT_NAME, "path": LOG_NAME, "maxLines": 3}),
...
def run_server(server: Path, policy: Path, protocol_input: str) -> dict[int, dict[str, Any]]:
    ...
    require("--allow-legacy-process-pinning" not in command, "legacy process mode was requested")
    ...
    for line in lines:
        ...
        require(response_id not in responses, "protocol output contains a duplicate response ID")
        require(response_id in expected_ids, "protocol output contains an unexpected response ID")
...
def result_content(response: dict[str, Any], label: str) -> dict[str, Any]:
    require(set(response) == {"id", "jsonrpc", "result"}, f"{label} response has an unexpected schema")
    require(response["jsonrpc"] == "2.0", f"{label} response has an unexpected JSON-RPC version")
    result = response["result"]
    require(isinstance(result, dict), f"{label} result is not an object")
    require(set(result) == {"content", "isError", "structuredContent"}, f"{label} result has an unexpected schema")
    require(result["isError"] is False, f"{label} returned a tool execution error")
    content = result["content"]
    structured = result["structuredContent"]
    require(isinstance(content, list) and len(content) == 1, f"{label} content is invalid")
    require(isinstance(content[0], dict) and content[0].get("type") == "text", f"{label} text content is invalid")
    require(isinstance(content[0].get("text"), str), f"{label} text content is not a string")
    try:
    ...
    require(isinstance(structured, dict), f"{label} structured content is not an object")
    return structured
...
def validate_log_common(value: dict[str, Any], expected_bytes: int, label: str) -> None:
    ...
    require(is_nonnegative_integer(value.get("bytesScanned")), f"{label} bytesScanned has an invalid type")
    require(value["bytesScanned"] == expected_bytes, f"{label} scanned an unexpected byte count")
    require(is_nonnegative_integer(value.get("linesScanned")), f"{label} linesScanned has an invalid type")
    require(value["linesScanned"] == 5, f"{label} scanned an unexpected line count")
    require(value.get("fileChangedDuringRead") is False, f"{label} reported a changed file")


def validate_search(value: dict[str, Any], expected_bytes: int, expected_lines: list[int], expected_previews: list[str], label: str) -> None:
    validate_log_common(value, expected_bytes, label)
    require(value.get("caseSensitive") is True, f"{label} did not use case-sensitive matching")
    require(value.get("matchLimitReached") is False, f"{label} reached its match limit")
    matches = value.get("matches")
    require(isinstance(matches, list) and len(matches) == len(expected_lines), f"{label} returned an unexpected match count")
    for match, line_number, preview in zip(matches, expected_lines, expected_previews):
        ...
        require(match.get("line") == line_number, f"{label} returned an unexpected line number")
        require(is_nonnegative_integer(match.get("byteOffset")), f"{label} returned an invalid byte offset")
        require(match.get("preview") == preview, f"{label} returned an unexpected preview")
        require(match.get("previewTruncatedStart") is False, f"{label} truncated the start of a preview")
        require(match.get("previewTruncatedEnd") is False, f"{label} truncated the end of a preview")


def validate_tail(value: dict[str, Any], expected_bytes: int, expected_lines: list[int], expected_previews: list[str]) -> None:
    validate_log_common(value, expected_bytes, "logs.tail")
    lines = value.get("lines")
    require(isinstance(lines, list) and len(lines) == 3, "logs.tail returned an unexpected line count")
    for line, line_number, preview in zip(lines, expected_lines, expected_previews):
        ...
        require(line.get("line") == line_number, "logs.tail returned an unexpected line number")
        require(is_nonnegative_integer(line.get("byteOffset")), "logs.tail returned an invalid byte offset")
        require(line.get("preview") == preview, "logs.tail returned an unexpected preview")
        require(line.get("previewTruncatedStart") is False, "logs.tail truncated a preview")
...
def validate_elf(value: dict[str, Any]) -> None:
    ...
    require(value.get("class") == "ELF64", "elf.inspect returned an unexpected ELF class")
    require(value.get("endianness") == "little", "elf.inspect returned an unexpected byte order")
    require(value.get("fileType") == "executable", "elf.inspect returned an unexpected file type")
    require(value.get("fileTypeNumber") == 2, "elf.inspect returned an unexpected file type number")
    require(value.get("machine") == "x86_64", "elf.inspect returned an unexpected machine")
    require(value.get("machineNumber") == 62, "elf.inspect returned an unexpected machine number")
    require(value.get("osAbi") == "linux", "elf.inspect returned an unexpected OS ABI")
    require(value.get("osAbiNumber") == 3, "elf.inspect returned an unexpected OS ABI number")
    require(value.get("entryPoint") == "0x0", "elf.inspect returned an unexpected entry point")
    require(value.get("programHeaderCount") == 0, "elf.inspect returned unexpected program headers")
    require(value.get("interpreter") is None, "elf.inspect returned an unexpected interpreter")
    require(value.get("neededLibraries") == [], "elf.inspect returned unexpected libraries")
    require(value.get("neededLibrariesTruncated") is False, "elf.inspect truncated libraries")
    require(value.get("buildId") is None, "elf.inspect returned an unexpected build ID")
    require(value.get("stackPolicy") == "unspecified", "elf.inspect returned an unexpected stack policy")
    require(value.get("relro") == "none", "elf.inspect returned an unexpected RELRO state")
    require(value.get("positionIndependent") is False, "elf.inspect returned an unexpected PIE signal")
    require(value.get("pieExecutable") is False, "elf.inspect returned an unexpected PIE executable signal")
    require(value.get("writableExecutableLoadSegment") is False, "elf.inspect returned an unexpected segment signal")
    require(value.get("fileChangedDuringRead") is False, "elf.inspect reported a changed file")
    require(is_nonnegative_integer(value.get("metadataBytesRead")), "elf.inspect returned an invalid metadata byte count")
    require(value.get("segments") == [], "elf.inspect returned unexpected segments")
    require(value.get("segmentSummariesTruncated") is False, "elf.inspect truncated segments")


def validate_process(value: dict[str, Any]) -> None:
    require(value.get("process") == "server", "proc.memory returned an unexpected process alias")
    require(is_integer(value.get("pid")) and value["pid"] > 0, "proc.memory returned an invalid PID")
    require(is_integer(value.get("uid")) and value["uid"] >= 0, "proc.memory returned an invalid UID")
    require(isinstance(value.get("name"), str) and value["name"], "proc.memory returned an invalid process name")
    require(isinstance(value.get("state"), str) and value["state"], "proc.memory returned an invalid process state")
    require(is_nonnegative_integer(value.get("threads")), "proc.memory returned an invalid thread count")
    require(is_integer(value.get("pageSizeBytes")) and value["pageSizeBytes"] > 0, "proc.memory returned an invalid page size")
    require(value.get("pidfdPinned") is True, "proc.memory did not report strict pidfd pinning")
    status = value.get("status")
    require(isinstance(status, dict), "proc.memory status counters are missing")
    for counter in ("vmSizeBytes", "vmRssBytes", "rssAnonBytes", "rssFileBytes", "rssShmemBytes"):
        require(is_nonnegative_integer(status.get(counter)), f"proc.memory counter {counter} is missing")
    statm = value.get("statm")
    require(isinstance(statm, dict), "proc.memory statm counters are missing")
    for counter in ("virtualBytes", "residentBytes", "sharedBytes", "textBytes", "dataAndStackBytes"):
        require(is_nonnegative_integer(statm.get(counter)), f"proc.memory counter {counter} is missing")
    require(isinstance(value.get("smapsRollupAvailable"), bool), "proc.memory rollup availability has an invalid type")
    require(value.get("smapsRollupError") is None or isinstance(value.get("smapsRollupError"), str), "proc.memory rollup error has an invalid type")


def report(responses: dict[int, dict[str, Any]], messages: list[dict[str, Any]], tool_calls: list[dict[str, Any]], log_bytes: bytes) -> dict[str, Any]:
    lines = log_bytes.decode("utf-8").splitlines()
    validate_initialize = responses[1]
    require(validate_initialize["result"]["serverInfo"]["version"] == VERSION, "server reported an unexpected version")
    require([tool["name"] for tool in responses[2]["result"]["tools"]] == EXPECTED_TOOLS, "server returned an unexpected tool list")
    ...
    validate_search(correlation, len(log_bytes), [1, 2, 3, 4, 5], lines, "correlation search")
    ...
    for message in messages:
        if "id" in message:
            request_records.append({"arguments": message.get("params", {}), "method": message["method"], "requestId": message["id"]})
    evidence = [
        {"arguments": tool_calls[0]["params"]["arguments"], "finding": {"expectedLines": [1, 2, 3, 4, 5], "matchCount": 5}, "requestId": 10, "tool": "logs.search"},
        {"arguments": tool_calls[1]["params"]["arguments"], "finding": {"expectedLines": [2], "matchCount": 1}, "requestId": 11, "tool": "logs.search"},
        {"arguments": tool_calls[2]["params"]["arguments"], "finding": {"expectedLines": [3, 4, 5], "finalState": "healthy"}, "requestId": 12, "tool": "logs.tail"},
        {"arguments": tool_calls[3]["params"]["arguments"], "finding": {"elfClass": "ELF64", "machine": "x86_64", "fileType": "executable"}, "requestId": 13, "tool": "elf.inspect"},
        {"arguments": tool_calls[4]["params"]["arguments"], "finding": {"observed": True, "requiredAggregateCountersPresent": True, "strictPidfdPinning": True}, "requestId": 14, "tool": "proc.memory"},
    ]
    return {"conclusion": "healthy_final_state_confirmed", "evidence": evidence, "investigation": {"correlationId": CORRELATION_ID, "scenario": "service_restart_authentication_recovery", "version": VERSION}, "requests": request_records, "schemaVersion": 1, "security": {"legacyFlagsPassed": False, "strictOpenat2": True, "strictPidfdPinning": True}}
...
def run_demo(server: Path, fixture: Path, output_dir: Path) -> None:
    ...
    try:
        ...
        require(canonical["conclusion"] == "healthy_final_state_confirmed", "the investigation conclusion changed")
...
def main() -> int:
    ...
    try:
        run_demo(arguments.server.resolve(), arguments.fixture.resolve(), arguments.output_dir.resolve())
